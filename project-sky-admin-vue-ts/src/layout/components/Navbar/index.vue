  ...
  <div class="navbar">
    ...
    <div :key="restKey"
      ...
      <div class="rightStatus">
        ...
        <span class="navicon addressSetting" @click="handleAddress"><i />店铺地址设置</span>
      </div>
    ...
    <!-- end -->
    <!-- 店铺地址设置 -->
    <el-dialog title="店铺地址设置"
               v-model="addressDialogVisible"
               width="460px"
               :show-close="false">
      <el-form label-position="top">
        <el-form-item label="店铺完整地址">
          <el-input v-model.trim="shopAddress"
                    type="textarea"
                    :rows="3"
                    maxlength="200"
                    show-word-limit
                    placeholder="例如：北京市海淀区上地十街10号" />
        </el-form-item>
        <p class="addressTip">该地址用于计算用户收货地址是否在配送范围内，请填写可被地图准确识别的完整地址。</p>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button @click="addressDialogVisible = false">取 消</el-button>
        <el-button type="primary" :loading="addressSaving" @click="handleAddressSave">保 存</el-button>
      </span>
    </el-dialog>
    <!-- end -->
    <!-- 修改密码 -->
...
export default class SkyComponent extends Vue {
  ...
  webSocket() {
    ...
    let socketUrl = import.meta.env.VITE_SOCKET_URL + clientId
  ...
  async handleAddress() {
    try {
      const { data } = await getShopAddress()
      if (data.code !== 1) {
        this.$message.error(data.msg)
        return
      }
      this.shopAddress = data.data || ''
      this.addressDialogVisible = true
    } catch (err) {
      this.$message.error('获取店铺地址失败')
    }
  }
  async handleAddressSave() {
    if (!this.shopAddress || this.shopAddress.trim().length < 5) {
      this.$message.warning('请输入完整的店铺地址')
      return
    }
    this.addressSaving = true
    try {
      const { data } = await setShopAddress(this.shopAddress.trim())
      if (data.code === 1) {
        this.addressDialogVisible = false
        this.$message.success('店铺地址保存成功')
      } else {
        this.$message.error(data.msg)
      }
    } catch (err) {
      this.$message.error('保存店铺地址失败')
    } finally {
